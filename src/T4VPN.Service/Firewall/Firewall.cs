...
namespace T4VPN.Service.Firewall
{
    internal class Firewall : IFirewall
    {
        ...
        private void BlockDns(uint weight)
        {
            _ipLayer.ApplyToIpv4(layer =>
            {
                _sublayer.CreateRemoteTcpPortFilter(new DisplayData(
                        "Time4VPS VPN block DNS", "Block TCP 53 port"),
                    Action.HardBlock,
                    layer,
                    weight,
                    53);
        ...
        private void PermitDhcp(uint weight)
        {
            _ipLayer.ApplyToIpv4(layer =>
            {
                _sublayer.CreateRemoteUdpPortFilter(
                    new DisplayData("Time4VPS VPN permit DHCP", "Permit 67 UDP port"),
                    Action.SoftPermit,
                    layer,
                    weight,
                    67);
        ...
        private void PermitFromNetworkInterface(string id, uint weight)
        {
            _ipLayer.ApplyToIpv4(layer =>
            {
                _sublayer.CreateNetInterfaceFilter(
                    new DisplayData("Time4VPS VPN permit VPN tunnel", "Permit TAP adapter traffic"),
                    Action.SoftPermit,
                    layer,
                    weight,
                    id);
        ...
        private void PermitOpenVpnServerAddress(string address)
        {
            ...
            _ipLayer.ApplyToIpv4(layer =>
            {
                filterGuids.Add(_sublayer.CreateRemoteIPv4Filter(
                    new DisplayData("Time4VPS VPN permit OpenVPN server", "Permit server ip"),
                    Action.HardPermit,
                    layer,
                    1,
                    address));
        ...
        private void BlockAllIpv4Network(uint weight)
        {
            _ipLayer.ApplyToIpv4(layer =>
            {
                _sublayer.CreateLayerFilter(
                    new DisplayData("Time4VPS VPN block IPv4", "Block all IPv4 traffic"),
                    Action.SoftBlock,
                    layer,
                    weight);
            });
        ...
        private void BlockAllIpv6Network(uint weight)
        {
            ...
            {
                _sublayer.CreateLayerFilter(
                    new DisplayData("Time4VPS VPN block IPv6", "Block all IPv6 traffic"),
                    Action.SoftBlock,
                    layer,
                    weight);
            });
        ...
        private void PermitIpv4Loopback(uint weight)
        {
            _ipLayer.ApplyToIpv4(layer =>
            {
                _sublayer.CreateLoopbackFilter(
                    new DisplayData("Time4VPS VPN permit IPv4 loopback", "Permit IPv4 loopback traffic"),
                    Action.HardPermit,
                    layer,
                    weight);
            });
        ...
        private void PermitIpv6Loopback(uint weight)
        {
            ...
            {
                _sublayer.CreateLoopbackFilter(
                    new DisplayData("Time4VPS VPN permit IPv6 loopback", "Permit IPv6 loopback traffic"),
                    Action.HardPermit,
                    layer,
                    weight);
            });
        ...
        private void PermitPrivateNetwork(uint weight)
        {
            _ipLayer.ApplyToIpv4(layer =>
            {
                _sublayer.CreateRemoteNetworkIPv4Filter(new DisplayData(
                        "Time4VPS VPN permit private network",
                        ""),
                    Action.HardPermit,
                    layer,
                    weight,
        ...
        private void PermitFromService(uint weight)
        {
            ...
            {
                _sublayer.CreateAppFilter(
                    new DisplayData("Time4VPS VPN permit service", "Permit Time4VPS VPN Service to bypass VPN tunnel"),
                    Action.HardPermit,
                    layer,
                    weight,
        ...
        private void PermitFromUpdateService(uint weight)
        {
            ...
            {
                _sublayer.CreateAppFilter(
                    new DisplayData("Time4VPS VPN permit update service", "Permit Time4VPS VPN Update Service to bypass VPN tunnel"),
                    Action.HardPermit,
                    layer,
                    weight,
